...
```{r import_data}
Path <- "D:/University/SMU/Statistical_Foundations_for_Data_Science/Stat_foundations_repository/Statistical_Foundations/Unit1/Creativity.csv"
df_create = read.csv(Path, header=TRUE)
df_create
```
## Calculate the Observed Difference in Means of the intristic vice extristic

This code calculates and records the difference in means: xbarIntrinsic - xbarExtrinsic = 4.144203

```{r calculate_mean_diff}
...
CreativityMeanDiff = xbars[1,2] - xbars[2,2] # observed difference xbarIntrinsic - xbarExtrinsic = 21.16667
CreativityMeanDiff
...
Assuming that the "New" method has no effect on the scores, each person should score the same on the test no matter if they were placed in the intrinsic or extrinsic group.  That is, if the "intrinsic" method has no effect on the scores then the distribution of scores from the population if everyone in the population was put in the "intrinsic" group is the exact same (equal) as the distribution of scores from the population if everyone in the population was put in the "Traditional" group.  This means that the means of those distributions are equal.  This is exactly what the null hypothesis is saying: Ho: mu_intrinsic = mu_extrinsic $\mu_{New} = \mu_{Traditional}$ or equivalently  $\mu_{New} - \mu_{Traditional} = 0$.
...
df
g <- df %>% 
  ggplot(mapping = aes(x = xbarDiffs)) + 
  geom_histogram(aes(y = after_stat(density)),bins = 25, fill = "cornflowerblue", linewidth = 0.1) 
g
...
# combn() - combination function finds all the different ways to assign the variables to the new groups

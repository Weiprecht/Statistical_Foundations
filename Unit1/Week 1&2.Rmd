...
Database <- data.frame(read.csv("D:/University/SMU/Statistical_Foundations_for_Data_Science/Stat_foundations_repository/Statistical_Foundations/Unit1/Creativity.csv"))
...
```{r filter_verb}
# Select cty column and filter rows with class "compact"
result <- mpg %>%
  select(cty, class, year) %>%
...
```{r NA_wrangling}
# age vectors
age = c(22,21,NA,19,20,23)
# years of math vectors
yrs_math_ed = c(4,5,NA,5,3,5)
...
df7 = data.frame(Age = age, Years = yrs_math_ed, Name = names, Subject = subject)
# ploat as a barplot with years on the y axis and the x axis divided by names
barplot(df7$Years, names.arg = df3$Name, ylab="Math Years", main="Math Years")
df7
is.na(df7$Years)
is.na(df7$Name)
is.na(df7$Major)
...
filter_df7 <-df7 %>%
  # include any column that does not have na
  filter(!is.na(Years))
filter_df7
```
...
```{r ggally_intro}
# Compare the columns against each other.
mpg %>%
...
```{r csv_import2}
Path <- "D:/University/SMU/Doing_Data_Science/DDS_repository/Doing-Data-Science/Unit3/FIFA_Players.csv"
FIFA = read.csv(Path, header=TRUE)
FIFA
